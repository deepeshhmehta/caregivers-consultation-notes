...
function addOther(name, field, val, form) {
...
function addNewOther(name, field, val, form) {
    if (jQuery('#'+form+' #' + name + '-1').length == 0) {
        if (val == 0) {
            var htmlStr = '';
            htmlStr = "<div id='" + name + "-1'><label class='item item-input item-floating-label'><input type='text' name='new-" + name + "' ng-model='records." + name + "' placeholder='Please Enter " + field + "' required /></label></div>";
            $("#"+form+ " [name='" + name + "']").parent().parent().append(htmlStr);//.append('<button type="button" class="button button-small addbtn" onclick="removeOther(\'' + name + '-' + 1 + '\',this)" id="rem-' + name + '-1" >Remove</button>');
        } else {
            jQuery('#' + name + '-1').remove();
            jQuery('#rem-' + name + '-1').remove();
        }
